### Creating Your Own MCP Server Using Python
...
To provide a hands-on example, let's implement a server that gets the current time and lists files in a directory:

1. **Setup**: Ensure the SDK is installed (`pip install mcp`). You'll need `datetime` and `os` for this example.
2. **Server Code**: Create `time_and_files_server.py`:
...
3. **Run and Test**:
   - Start in development mode: `mcp dev time_and_files_server.py`.
   - Test with MCP Inspector: `mcp inspector`. Call `get_current_time` to see the current time or `list_files` with a directory path to list files.
   - Install for Claude Desktop: `mcp install time_and_files_server.py`, then use in Claude Desktop for LLM interactions.

This implementation demonstrates both tools (actions) and resources (data access), enhancing AI capabilities with real-world functionality.
